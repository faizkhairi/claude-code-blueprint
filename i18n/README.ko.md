...
**12 agents** · **19 skills** · **15 hooks** · **6 rules**, 실제 프로젝트에서 검증됨
...
다른 저장소는 **수십 개의 agents**를 제공합니다. 우리는 **12개**를 제공하며, 각각이 왜 존재하는지 설명합니다.
...
| [55개 자동 테스트](../hooks/test-hooks.sh)를 통한 [스모크 테스트 hooks](../hooks/test-hooks.sh) | 테스트되지 않은 스크립트 |
...
<details>
<summary><strong>19 Skills</strong>: 자연어 트리거 워크플로우(슬래시 명령 필요 없음)</summary>
...
| Planning | sprint-plan, elicit-requirements, self-audit | "빌드하자", "새로운 기능", "자체 감사", "전부 확인했어?" |
...
<details>
<summary><strong>15 Hooks</strong>: 결정적인 라이프사이클 자동화(100% 준수, CLAUDE.md 규칙은 항상 따르지 않는 것과 달리)</summary>
...
추가 2개 유틸리티 스크립트: `verify-mcp-sync.sh` (MCP 설정 확인) 및 `status-line.sh` (브랜치/프로젝트 상태), 그리고 컴패니언 도구 `check-no-dash-file.py` (`no-dash-check.sh`용 게시 전 프로즈 검사). 유틸리티는 full 프리셋으로 배포됩니다. 폴더의 마지막 파일은 `test-hooks.sh`이며, 로컬 테스트 하네스로 `bash hooks/test-hooks.sh`로 실행하여 모든 hooks를 검증합니다. 이 파일만 `~/.claude/hooks/`에 배포되지 않으며, "15 hooks" 총합에도 포함되지 않습니다.

`bash hooks/test-hooks.sh`를 실행하여 모든 hooks가 통과하는지 확인하세요 (55개 자동 테스트).
...
4. **Hooks는 무료, 컨텍스트는 저렴**: 15개 hook 스크립트는 토큰 비용 제로입니다 (Claude 컨텍스트 외부에서 실행). CLAUDE.md는 세션당 약 2,300 토큰을 추가합니다. 일반 세션의 약 1-5%입니다. 블루프린트는 재시도 사이클 방지로 비용 이상의 토큰을 절약합니다. [BENCHMARKS.md](../docs/BENCHMARKS.md#token-cost-per-component) 참조.
