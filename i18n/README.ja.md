...
**12 agents** · **19 skills** · **15 hooks** · **6 rules**、実際のプロジェクトで検証済み
...
他のリポジトリは **何十もの agent** を大量に提供します。私たちは **12個** だけを提供し、それぞれが存在する理由を説明します。
...
| [スモークテスト済み hook](../hooks/test-hooks.sh)：55個の自動テスト | テストなしのスクリプト |
...
<details>
<summary><strong>12 Agents</strong>：モデル層別（opus/sonnet/haiku）の専用 sub-agent</summary>
...
| memory-curator | sonnet | メモリディレクトリの孤立ファイル、インデックスのドリフト、リンク切れ、古いエントリを監査（レポートのみ） |
...
<details>
<summary><strong>19 Skills</strong>：自然言語トリガーのワークフロー（スラッシュコマンド不要）</summary>
...
| PR レビュー | pr-review | "この PR をレビュー"、"PR #N をレビュー"、"PR レビューを投稿" |
| テスト | test-check、e2e-check | "テスト実行"、"ブラウザテスト"、"テストパスしてますか？" |
| デプロイ | deploy-check | "デプロイ"、"本番へプッシュ"、"リリース準備完了" |
| 計画 | sprint-plan、elicit-requirements、self-audit | "ビルドしよう"、"新機能"、"自己監査"、"全部確認した?" |
...
<details>
<summary><strong>15 Hooks</strong>：決定的なライフサイクル自動化（決定的に毎回発火、CLAUDE.md ルールは必ずしも守られない点と異なり）</summary>
...
プラス2つのユーティリティスクリプト：`verify-mcp-sync.sh`（MCP 設定チェッカー）と`status-line.sh`（ブランチ/プロジェクトステータス）、およびコンパニオンツール `check-no-dash-file.py`（`no-dash-check.sh` 用の投稿前プローズ検査）。ユーティリティは full プリセットでデプロイされます。フォルダ内の最後のファイルは `test-hooks.sh` で、ローカルテストハーネスとして `bash hooks/test-hooks.sh` で実行しすべての hook を検証します。これは `~/.claude/hooks/` にデプロイされない唯一のファイルであり、「15 hooks」の合計には含まれません。

すべての hook が動作することを確認するには `bash hooks/test-hooks.sh` を実行してください（55の自動テスト）。
...
4. **Hook は無料、コンテキストは安い**：15 個の hook スクリプトはトークンコストゼロです（Claude のコンテキスト外で実行されます）。CLAUDE.md はセッションあたり約 2,300 トークンを追加します。これは通常セッションの約 1-5% です。ブループリントはリトライサイクルの防止により、コスト以上のトークンを節約します。[BENCHMARKS.md](../docs/BENCHMARKS.md#token-cost-per-component) を参照してください。
