...
**自分のプロジェクトに組み込んで使う、コピペ可能なファイルのライブラリ（CLAUDE.md、hooks、agents） -- Claude Code をより信頼できるものに。**
...
**11 agents** · **17 skills** · **10 hooks** · **5 rules** -- 実際のプロジェクトで検証済み
...
2. **エージェント単位のナレッジ、グローバルな肥大化ではなく** -- デザイン原則は frontend agent に住むべきであり、毎セッション読み込むべきではありません。セキュリティパターンは security-reviewer に住むべきで、CLAUDE.md にはありません。
...
クローンまたはフォークしたコピーから `./setup.sh` を実行します。プリセット（minimal/standard/full）を選択すると、ディレクトリ作成、ファイルコピー、設定マージ、プレースホルダー置換を処理します。[SETUP.md](SETUP.md) を参照。
...
| 自動インストーラー + 検証チェックリスト | フレームワーク別 CLAUDE.md テンプレート | プロジェクトの方向性と今後の予定 |
