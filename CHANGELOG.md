...
- Repaired the sanitization gate in `.github/PULL_REQUEST_TEMPLATE.md`, which could never pass. It told contributors that `grep -riE '(internal\.example|real-project-name)' .` must return 0 hits, but four tracked files carry those strings deliberately (the template itself, `.lycheeignore`, `docs/SETTINGS-GUIDE.md`, and `hooks/block-git-push.sh`), so every contributor who ran the documented command saw hits and learned to ignore the check. The gate now scans only the files the PR changes, names the four intentional occurrences, and uses `xargs -r` so an empty file list does not silently fall back to recursing the whole repo.
