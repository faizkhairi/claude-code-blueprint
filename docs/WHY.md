...
**What we built:** A `PreToolUse` hook on Write|Edit that fires only on edits to `settings.json` and prompts (`decision:ask`) on two things. First, an edit touching a safety key (`defaultMode`, the `deny`/`ask` lists, `disableAllHooks`, `disableAutoMode`, or `bypassPermissions`). Second, an added `Edit()`/`Write()` allow rule pointing inside the `.claude` directory, because those entries do nothing: `permissions.allow` does not pre-approve writes to [protected paths](https://code.claude.com/docs/en/permission-modes#protected-paths), the safety check runs first, and the write is routed to the classifier (auto mode) or denied (`dontAsk`) regardless of what is listed. They accumulate on their own, since the prompt for a `.claude/` write offers to grant access for the session and that grant lands in `settings.json`. Left alone they read as permissions someone deliberately granted, which makes the allow list harder to audit for the entries that do matter. Other routine settings edits (a theme, an env var, an ordinary allow rule) pass through untouched, and removing an inert entry is never blocked.
